            ...
            foreach ($currentSitemapSites as $site) {
                ...
                foreach ($elements as $element) {
                    ...
                    if ($noIndex == 1 OR $noFollow == 1) {
                        SproutSitemaps::info(Craft::t('sprout-sitemaps', 'Element ID {elementId} not added to sitemap. Element Metadata field `noindex` or `nofollow` settings are enabled.', [
                    ...
                    if (!empty($canonicalOverride)) {
                        SproutSitemaps::info(Craft::t('sprout-sitemaps', 'Element ID {elementId} is using a canonical override and has not been included in the sitemap. Element URL: {elementUrl}. Canonical URL: {canonicalUrl}.', [
                            'elementId' => $element->id,
                    ...
                    if ($element->getUrl() === null) {
                        SproutSitemaps::info(Craft::t('sprout-sitemaps', 'Element ID {elementId} not added to sitemap. Element does not have a URL.', [
                            'elementId' => $element->id
                        ]));
                        continue;
                    }
