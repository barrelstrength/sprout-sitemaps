    ...
    Craft.SproutSeo.Sitemaps = Garnish.Base.extend(
        {
            ...
            createAndEditSitemapSection: function(event) {
                ...
                if (!this.submitStatus) {
                    ...
                    data = {
                        "redirect": 'sprout-sitemaps/sections/{id}',
                        "enabledForSite": true,
                        "sproutseo": {
                            "metadata": {
                                "name": $($row).data('name'),
                    ...
                    Craft.postActionRequest('sprout-sitemaps/sitemaps/save-sitemap-section', data, $.proxy(function(response, textStatus) {
                        if (textStatus == 'success') {
                            if (response.success) {
                                Craft.redirectTo('sprout-sitemaps/sections/' + response.sitemapSection.id);
                            }
                        }
                    }, this));
                }
                ...
                Craft.postActionRequest('sprout-sitemaps/sitemaps/save-sitemap-section', data, $.proxy(function(response, textStatus) {
                    if (textStatus === 'success') {
                        if (response.success) {
                            ...
                            Craft.cp.displayNotice(Craft.t('sprout-sitemaps', "Sitemap Metadata saved."));
                        }
                        else {
                            Craft.cp.displayError(Craft.t('sprout-sitemaps', 'Unable to save Sitemap Metadata.'));
                        }
                    }
                }, this));
